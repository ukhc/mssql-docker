...
echo "restore mssql..."
kubectl exec $POD -- bash -c "rm -rf /var/opt/mssql/*"
kubectl cp ./backup/$BACKUP_FOLDER/mssql $POD:/var/opt

##########################

echo "stop the mssql deployment..."
kubectl scale --replicas=0 deployment mssql
...
echo "start the mssql deployment..."
kubectl scale --replicas=1 deployment mssql
...
do
	isPodReady=$(kubectl get pod -l app=mssql -o jsonpath="{.items[0].status.containerStatuses[*].ready}")
	if [ "$isPodReady" != "true" ]; then
		((isPodReadyCount++))
		if [ "$isPodReadyCount" -gt "100" ]; then
			echo "ERROR: timeout waiting for mssql pod. Exit script!"
			exit 1
		else
			echo "waiting...mssql pod is not ready...($isPodReadyCount/100)"
			sleep 2
		fi
	fi
done
